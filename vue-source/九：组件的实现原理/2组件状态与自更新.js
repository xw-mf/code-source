...
function createRenderer(options) {
    ...
    function mountComponent(vnode, container, anchor) {
        ...
        // 组件的自更新。为此，我们需要将整个渲染任务包装到一个 effect中
        effect(() => {
            ...
            // 但是，由于 effect 的执行是同步
            // 的，因此当响应式数据发生变化时，与之关联的副作用函数会同步执
            // 行。换句话说，如果多次修改响应式数据的值，将会导致渲染函数执
            // 行多次，这实际上是没有必要的。因此，我们需要设计一个机制，以
            // 使得无论对响应式数据进行多少次修改，副作用函数都只会重新执行
            // 一次。为此，我们需要实现一个调度器，当副作用函数需要重新执行
            // 时，我们不会立即执行它，而是将它缓冲到一个微任务队列中，等到
            // 执行栈清空后，再将它从微任务队列中取出并执行
            scheduler: queueJob
        })
    }
