    ...
    function patchKeyedChildren(n1, n2, container) {
        ...
        while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
            ...
            if (!oldStartVnode) {
                ...
                // 第五步：在四个步骤的比较过程中，都无法找到可复用的节点，应该怎么
                // 办呢？这时，我们只能通过增加额外的处理步骤来处理这种非理想情
                // 况。既然两个头部和两个尾部的四个节点中都没有可复用的节点，那
                // 么我们就尝试看看非头部、非尾部的节点能否复用。具体做法是，拿
        ...
        if (oldEndIdx < oldStartIdx && newStartIdx <= newEndIdx) {
            ...
            // 索引值位于 newStartIdx 和 newEndIdx 这个区间内的节点都是新节点
            for (let i = newStartIdx; i <= newEndIdx; i++) {
                patch(null, newChildren[i], container, oldStartVnode.el)
            }
        } else if (newEndIdx < newStartIdx && oldStartIdx <= oldEndIdx) {
            // 索引值位于 oldStartIdx 和 oldEndIdx 这个区间内的节点都应该被卸载
